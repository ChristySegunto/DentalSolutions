...
const Edittreatment = () => {
    ...
    const [currentFile, setCurrentFile] = useState({ treatment_filename: '', treatment_filedescription: '' });
    ...
    useEffect(() => {
        ...
        const fetchTreatmentDetails = async () => {
            try {
                const { data, error } = await supabase
                    ...
                    .select('*')
                    .eq('treatment_id', parseInt(treatment_id))
                    .single();

                if (error) throw error;

                setTreatment(data);
                setActiveToothButton(data.treatment_toothnum);
                setSelectedTreatment(data.treatment);
                fetchTreatmentTypes(data.treatment);
                setSelectedTreatmentType(data.treatment_type);
                setNextVisitDate(data.treatment_nextvisit);

            } catch (error) {
                console.error('Error fetching treatment details:', error.message);
                setUserError('Failed to fetch treatment details');
            }
        };

        const fetchTreatmentPrescription = async () => {
            try {
                const { data, error } = await supabase
                ...
                setUserError('Failed to fetch prescription details');
            }
        };
        ...
        const fetchTreatmentFile = async () => {
            try {
                ...
                console.error('Error fetching treatment files:', error.message);
                setUserError('Failed to fetch treatment files');
            }
        };

        fetchPatientDetails();
        fetchTreatmentDetails();
        fetchTreatmentPrescription();
        fetchTreatmentFile();
        setLoading(false);
    ...
    //UPDATING TREATMENT
        try{
            ...
            try{
                ...
                const newPrescriptions = prescriptions.filter(p => {
                    const exists = existingPrescriptions.some(ep => (
                        ep.treatment_medicine === p.treatment_medicine &&
                        ep.treatment_reason === p.treatment_reason &&
                        ep.treatment_duration === p.treatment_duration &&
                        ep.treatment_taken === p.treatment_taken
                    ));
                    return !exists;
                });
                ...
                const prescriptionsToUpsert = newPrescriptions.map(p => ({
                    ...
                    treatment_medicine: p.treatment_medicine,
                    treatment_reason: p.treatment_reason,
                    treatment_duration: p.treatment_duration,
                    treatment_taken: p.treatment_taken
                }));
                ...
                try {
                    ...
                    const newFiles = files.filter(f => {
                        const exists = existingFiles.some(ef => (
                            ef.treatment_filename === f.treatment_filename &&
                            ef.treatment_filedescription === f.treatment_filedescription
                        ));
                        return !exists;
                    });
                    ...
                    const filesToUpsert = newFiles.map(f => ({
                        patient_id,
                        treatment_id: treatment_id,
                        treatment_filename: f.treatment_filename,
                        treatment_filedescription: f.treatment_filedescription,
                    ...
                    if (filesToUpsert.length > 0) {
                        ...
                        for (let file of files) {
                            const { data, error: uploadError } = await supabase.storage
                                ...
                                .upload(`public/${patient_id}/${treatment_id}/${file.treatment_filename}`, file.file);
                        }
            ...
            setUserError('Failed to update treatment');
        }
    };
...
//HANDLE PRESCRIPTION
    const validatePrescriptionData = (currentPrescription) => {
        return(
            currentPrescription.treatment_medicine !== '' &&
            currentPrescription.treatment_reason !== '' &&
            currentPrescription.treatment_duration !== '' &&
            currentPrescription.treatment_taken !== ''
        );
    }
...
//HANDLE TREATMENT
    const handleTreatmentDataChange = (event) => {
        const { name, value } = event.target;
        if (name === 'treatment_nextvisit') {
            setNextVisitDate(value); // Allow manual change of next visit date
        } else {
...
//HANDLE FILE
    ...
    const handleFileChange = (event) => {
        ...
        setCurrentFile(prevState => ({
            ...prevState,
            [name === 'filename' ? 'treatment_filename' : 'treatment_filedescription']: value
        }));
    };
    ...
    const handleAddFile = () => {
        if (currentFile.treatment_filename && currentFile.file) {
            const existingFile = files.find(file => file.treatment_filename === currentFile.treatment_filename);
    ...
    const handleDeleteFile = async () => {
        ...
        try{
            ...
            const { data: existingFile, error: fetchError } = await supabase
                .from('treatment_files')
                .select('*')
                .eq('treatment_filename', fileToDelete.treatment_filename)
                .single();
            ...
            if (existingFile) {
                ...
                const { error: deleteStorageError } = await supabase
                    .storage
                    .from('treatment_fileupload')
                    .remove([`public/${patient_id}/${treatment_id}/${fileToDelete.treatment_filename}`]);
...
//HANDLE EXPORT
    const handleExport =  () => {
        ...
        prescriptions.forEach((p, index) => {
            ...
            doc.setFontSize(12);
            doc.text(`Medicine: ${p.treatment_medicine}`, 70, yPos + 6);
            doc.text(`Reason: ${p.treatment_reason}`, 70, yPos + 11);
            doc.text(`Duration: ${p.treatment_duration}`, 70, yPos + 16);
            doc.text(`Taken: ${p.treatment_taken}`, 70, yPos + 21);
        ...
        <div className="editprocedure-content container-fluid">
                ...
                <div className="container toothform">
                    <div className="row justify-content-md-center">
                        <div className="col col-lg-auto col-md-auto col-sm-auto col-auto g-0">
                            <Button className={`toothbtn ${activeToothButton === '18' ? 'active' : ''}` } onClick={() => handleToothNumber('18')}>
                                <label className="row d-flex justify-content-center">18</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '17' ? 'active' : ''}`} onClick={() => handleToothNumber('17')}>
                                <label className="row d-flex justify-content-center">17</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '16' ? 'active' : ''}`} onClick={() => handleToothNumber('16')}>
                                <label className="row d-flex justify-content-center">16</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '15' ? 'active' : ''}`} onClick={() => handleToothNumber('15')}>
                                <label className="row d-flex justify-content-center">15</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '14' ? 'active' : ''}`} onClick={() => handleToothNumber('14')}>
                                <label className="row d-flex justify-content-center">14</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '13' ? 'active' : ''}`} onClick={() => handleToothNumber('13')}>
                                <label className="row d-flex justify-content-center">13</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '12' ? 'active' : ''}`} onClick={() => handleToothNumber('12')}>
                                <label className="row d-flex justify-content-center">12</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '11' ? 'active' : ''}`}  onClick={() => handleToothNumber('11')}>
                                <label className="row d-flex justify-content-center">11</label>
                        ...
                        <div className="col col-lg-auto col-md-auto col-sm-auto col-auto g-0">
                            <Button className={`toothbtn ${activeToothButton === '21' ? 'active' : ''}`} onClick={() => handleToothNumber('21')}>
                                <label className="row d-flex justify-content-center">21</label>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="30" height="106" viewBox="0 0 30 106" fill="none">
                            ...
                            <Button className={`toothbtn ${activeToothButton === '22' ? 'active' : ''}`} onClick={() => handleToothNumber('22')}>
                                <label className="row d-flex justify-content-center">22</label>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="25" height="107" viewBox="0 0 25 107" fill="none">
                            ...
                            <Button className={`toothbtn ${activeToothButton === '23' ? 'active' : ''}`} onClick={() => handleToothNumber('23')}>
                                <label className="row d-flex justify-content-center">23</label>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="29" height="107" viewBox="0 0 29 107" fill="none">
                            ...
                            <Button className={`toothbtn ${activeToothButton === '24' ? 'active' : ''}`} onClick={() => handleToothNumber('24')}>
                                <label className="row d-flex justify-content-center">24</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '25' ? 'active' : ''}`} onClick={() => handleToothNumber('25')}>
                                <label className="row d-flex justify-content-center">25</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '26' ? 'active' : ''}`} onClick={() => handleToothNumber('26')}>
                                <label className="row d-flex justify-content-center">26</label>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="40" height="108" viewBox="0 0 40 108" fill="none">
                            ...
                            <Button className={`toothbtn ${activeToothButton === '27' ? 'active' : ''}`} onClick={() => handleToothNumber('27')}>
                                <label className="row d-flex justify-content-center">27</label>
                            ...
                            <Button className={`toothbtn ${activeToothButton === '28' ? 'active' : ''}`} onClick={() => handleToothNumber('28')}>
                                <label className="row d-flex justify-content-center">28</label>
                    ...
                    <div className="row justify-content-md-center mt-2">
                        <div className="col col-lg-auto col-md-auto col-sm-auto col-auto g-0">
                        <Button className={`toothbtn ${activeToothButton === '48' ? 'active' : ''}`} onClick={() => handleToothNumber('48')}>
                                ...
                                <label className="row d-flex justify-content-center">48</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '47' ? 'active' : ''}`} onClick={() => handleToothNumber('47')}>
                                ...
                                <label className="row d-flex justify-content-center">47</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '46' ? 'active' : ''}`} onClick={() => handleToothNumber('46')}>
                                ...
                                <label className="row d-flex justify-content-center">46</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '45' ? 'active' : ''}`} onClick={() => handleToothNumber('45')}>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="27" height="107" viewBox="0 0 27 107" fill="none">
                                ...
                                <label className="row d-flex justify-content-center">45</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '44' ? 'active' : ''}`} onClick={() => handleToothNumber('44')}>
                                ...
                                <label className="row d-flex justify-content-center">44</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '43' ? 'active' : ''}`} onClick={() => handleToothNumber('43')}>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="27" height="108" viewBox="0 0 27 108" fill="none">
                                ...
                                <label className="row d-flex justify-content-center">43</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '42' ? 'active' : ''}`} onClick={() => handleToothNumber('42')}>
                                ...
                                <label className="row d-flex justify-content-center">42</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '41' ? 'active' : ''}`} onClick={() => handleToothNumber('41')}>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="19" height="108" viewBox="0 0 19 108" fill="none">
                                ...
                                <label className="row d-flex justify-content-center">41</label>
                            </Button>
                        </div>

                        <div className="col col-lg-auto col-md-auto col-sm-auto col-auto g-0">
                            <Button className={`toothbtn ${activeToothButton === '31' ? 'active' : ''}`} onClick={() => handleToothNumber('31')}>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="19" height="108" viewBox="0 0 19 108" fill="none">
                                ...
                                <label className="row d-flex justify-content-center">31</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '32' ? 'active' : ''}`} onClick={() => handleToothNumber('32')}>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="19" height="108" viewBox="0 0 19 108" fill="none">
                                ...
                                <label className="row d-flex justify-content-center">32</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '33' ? 'active' : ''}`} onClick={() => handleToothNumber('33')}>
                                <svg className="row d-flex justify-content-center" xmlns="http://www.w3.org/2000/svg" width="27" height="108" viewBox="0 0 27 108" fill="none">
                                ...
                                <label className="row d-flex justify-content-center">33</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '34' ? 'active' : ''}`} onClick={() => handleToothNumber('34')}>
                                ...
                                <label className="row d-flex justify-content-center">34</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '35' ? 'active' : ''}`} onClick={() => handleToothNumber('35')}>
                                ...
                                <label className="row d-flex justify-content-center">35</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '36' ? 'active' : ''}`} onClick={() => handleToothNumber('36')}>
                                ...
                                <label className="row d-flex justify-content-center">36</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '37' ? 'active' : ''}`} onClick={() => handleToothNumber('37')}>
                                ...
                                <label className="row d-flex justify-content-center">37</label>
                            </Button>
                            <Button className={`toothbtn ${activeToothButton === '38' ? 'active' : ''}`} onClick={() => handleToothNumber('38')}>
                                ...
                                <label className="row d-flex justify-content-center">38</label>
                            </Button>
                        </div>
                    </div>
                </div>
            ...
            <div className="addtreatmentform">
                <div className="treatmentdetails row">
                    ...
                    <Form.Group className="col-lg-6 col-md-12 mb-3" controlId="formBasicName">
                        ...
                        <Form.Control type="date" value={treatment.treatment_date} name="treatment_date" disabled/>
                    </Form.Group>

                    <Form.Group className="col-lg-6 col-md-12 mb-3" controlId="formBasicName">
                        <Form.Label className="txtbox-lbl">Next Visit</Form.Label>
                        <Form.Control type="date" value={nextVisitDate} onChange={handleTreatmentDataChange} name="treatment_nextvisit" />
                    </Form.Group>

                    <Form.Group className="col-lg-6 col-md-12 mb-3" controlId="formBasicName">
                        <Form.Label className="txtbox-lbl">Dentist</Form.Label>
                        <Form.Control type="text" placeholder={treatment.treatment_dentist} value={treatment.treatment_dentist} name="treatment_dentist" disabled/>
                    </Form.Group>
                    ...
                    <Form.Group className="col-lg-6 col-md-12 mb-3" controlId="formBasicProcedure">
                        ...
                        <Form.Control className="custom-text-form" type="text" placeholder={treatment.treatment_description} name="treatment_description" value={treatment.treatment_description} onChange={handleTreatmentDataChange}/>
                    </Form.Group>
                </div>
            </div>
                        ...
                        <tbody>
                            {prescriptions.map((p, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="col-lg-3 custom-td align-content-center">{p.treatment_medicine}</td>
                                    <td className="col-lg-3 custom-td align-content-center">{p.treatment_reason}</td>
                                    <td className="col-lg-2 custom-td align-content-center">{p.treatment_duration}</td>
                                    <td className="col-lg-2 custom-td align-content-center">{p.treatment_taken}</td>
                    ...
                    <Table bordered hover responsive>
                        <thead className="custom-table-head">
                            ...
                            <tbody>
                                ...
                                <tr key={index}>
                                    <td className="align-content-center">{index + 1}</td>
                                    <td className="align-content-center">{file.treatment_filename}</td>
                                    <td className="align-content-center">{file.treatment_filedescription}</td>
                                    <td className="col-lg-1 custom-td text-center align-content-center">
                ...
                <Modal.Body>
                    <Form.Group className="mb-3 prescriptionformbody" controlId="formBasicMedicine">
                        ...
                        <Form.Control 
                            ...
                            value={currentPrescription.treatment_medicine}
                    ...
                    <Form.Group className="mb-3 prescriptionformbody" controlId="formBasicReason">
                        ...
                        <Form.Control 
                            ...
                            value={currentPrescription.treatment_reason}
                            onChange={handlePrescriptionChange}
                        />
                    </Form.Group>
                    <Form.Group className="mb-3 prescriptionformbody" controlId="formBasicDuration">
                        ...
                        <Form.Control 
                            ...
                            value={currentPrescription.treatment_duration}
                            onChange={handlePrescriptionChange}
                        />
                    </Form.Group>
                    <Form.Group className="mb-3 prescriptionformbody" controlId="formBasicDuration">
                        ...
                        <Form.Control 
                            ...
                            value={currentPrescription.treatment_taken}
                            onChange={handlePrescriptionChange}
                        />
                    </Form.Group>
                ...
                <Modal.Body>
                    <Form.Group className="mb-3 fileformbody" controlId="formBasicFileName">
                        ...
                        <Form.Control 
                            ...
                            name="filename"
                            value={currentFile.treatment_filename}
                    ...
                    <Form.Group className="mb-3 fileformbody" controlId="formBasicFileDescription">
                        ...
                        <Form.Control 
                            ...
                            name="description"
                            value={currentFile.treatment_filedescription}
                            onChange={handleFileChange}
                        />
                    </Form.Group>
            ...
            <Modal show={showSuccessMessage} onHide={() => setShowSuccessMessage(false)} centered>
                ...
                <Modal.Body className="custom-modal-body text-center">
                    The treatment has been successfully updated.<br/>
                    Redirecting to View Treatment page...
                </Modal.Body>
            </Modal>
        </div>
        </>
